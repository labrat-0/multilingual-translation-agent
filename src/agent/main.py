...
ISO_CODE_PATTERN = re.compile(r"^[a-z]{2}$", re.IGNORECASE)
MAX_TEXT_LENGTH = 5000


def is_valid_iso639_1(code: str | None) -> bool:
    if not code:
        return False
...
async def main() -> None:
    async with Actor:
        ...
        source_lang = normalize_code(actor_input.get("source_language", "en")) if actor_input.get("source_language") else "auto"
        ...
        if len(text) > MAX_TEXT_LENGTH:
            await Actor.fail(f"Text exceeds maximum length of {MAX_TEXT_LENGTH} characters.")
            return

        if not is_valid_iso639_1(target_lang):
            await Actor.fail(f"Invalid target language code '{target_lang}'. Must be ISO 639-1.")
            return

        if source_lang != "auto" and not is_valid_iso639_1(source_lang):
            await Actor.fail(f"Invalid source language code '{source_lang}'. Must be ISO 639-1.")
            return
